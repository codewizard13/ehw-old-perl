...
sub initMessage
{
   ...
   $message += " of the string (like a huge negative number bigger";
   $message += " than the length of the string), the beginning of";
   $message += " the string is the start position (as if you had";
   $message += " used 0 for a starting position). If the start";
   $message += " position is a huge positive number, the empty";
   $message += " string is always returned. In other words, it";
   $message += " probably does what you expect it to do, as long";
   $message += " as you expect it to always return something";
   $message += " other than an error.";
...
sub makePrelimDocumentation
{
   ...
   print "\n";
   #
   print "Enter Program Description (CTRL-D to Quit):\n\n";
   @progDescr = <STDIN>;
   chomp( @progDescr );
   ...
   open( HOLDER, ">$documentationHolder");
      ...
      print HOLDER "*   \n";
      print HOLDER "*   Purpose:    @progDescr\n";
...
#      print "\$remainingDocChars =        $remainingDocChars\n";
#      print "\$charsInLine =              $charsInLine\n";
...
sub wrap
{
   ...
   while ( $strLen > $len )
   {
      ...
      if ( $charCount % $len == 0  && $charCount != 0 )
      {
         ...
         $restOfStr = substr( $restOfStr, $len, 1000 );
